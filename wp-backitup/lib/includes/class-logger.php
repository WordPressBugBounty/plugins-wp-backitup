...
class WPBackItUp_Logger {
	...
	private static function getLogger($log_name, $log_level = LogLevel::DEBUG) {
		...
		$log_hash_transient_key='log_hash_key';
		$log_hash_value = get_transient($log_hash_transient_key);
		if ($log_hash_value === false) {
			$log_hash_value = md5( date( 'Y-m-d H:i:s' ) . '_' . mt_rand());

			//Expire hash every hour
			$expiration = HOUR_IN_SECONDS;
			set_transient( $log_hash_transient_key, $log_hash_value, $expiration );
			//error_log('Refresh Hash:'. $log_hash_value);
		}

		//Trim long log name just in case
		$log_name=mb_substr($log_name.'_'.$log_hash_value,0, 250);
        $options = array (
            'extension'      => 'log',
